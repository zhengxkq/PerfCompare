...
export function useSolutions() {
  ...
  const fetchSolutions = async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        // 如果返回空数组，使用fallback数据
        solutions.value = solutionsDataFallback
      }
    } catch (error) {
      console.error('Failed to fetch solutions, using fallback data:', error)
      // 使用fallback数据
      solutions.value = solutionsDataFallback
    } finally {
      loading.value = false
  ...
  // 从API获取单个解决方案
  const fetchSolutionById = async (id: string) => {
    try {
      const data = await $fetch<OptimizationSolution>(`/api/solutions/${id}`)
      return data
    } catch (error) {
      console.error('Failed to fetch solution:', error)
      // 使用fallback数据
      return solutionsDataFallback.find(s => s.id === id)
    }
  }
  ...
  const getSolutionsByCategory = (category: string) => {
    return solutions.value.filter(s => s.category === category)
  }
  
  const categories = computed(() => {
    return Array.from(new Set(solutions.value.map(s => s.category)))
  })
